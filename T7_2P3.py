...
class openbrew:
    ...
    # retrieving response if status code is success(200) and getting all page data
    ...
    def fetch_data_state(self,state):
        ...
        if self.api_status_code(state_url)==200:
            ...
            for data in self.fetch_webPage_data(state_url):
                web_link = str(data.get('website_url'))
                # Trying to get only breweries which are having website, so using below condition
                if data.get('state_province') == state and web_link.startswith("http"):
                    name = data.get('name')
                    count += 1
                    #printing name of the brewery present in the state                   
                    print(f"breweries present in the state {state} :{name} and its website link:{web_link}")
            # printing total number of count of breweries
            print(f"Total number of breweries in {state} which are having website link is :{count}")
